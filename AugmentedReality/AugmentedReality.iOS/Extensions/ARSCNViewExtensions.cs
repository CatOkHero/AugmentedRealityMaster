...
namespace AugmentedReality.iOS.Extensions
{
	public static class ARSCNViewExtensions
	{
		...
		{
			if (self.Session == null || self.Session.CurrentFrame == null)
			{
				return null;
			}

			var currentFrame = self.Session.CurrentFrame;
		...
		public static HitTestRay HitTestRayFromScreenPos(this ARSCNView self, CGPoint point)
		{
			...
			var frame = self.Session.CurrentFrame;
			var cameraPos = SCNVector3.One;
			var rayDirection = SCNVector3.One;
			...
			try
			{
				...
				rayDirection = screenPosOnFarClippingPlane.Subtract(cameraPos);
		...
		public static FeatureHitTestResult[] HitTestWithFeatures(this ARSCNView self, CGPoint point, double coneOpeningAngleInDegrees, double minDistance = 0, double maxDistance = Double.MaxValue, int maxResults = 1)
		{
			...
			var features = self.Session.CurrentFrame.RawFeaturePoints;
			if (features == null)
			{
				return results.ToArray();
			}
